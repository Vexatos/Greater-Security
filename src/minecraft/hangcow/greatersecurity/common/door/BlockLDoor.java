...
public class BlockLDoor extends BlockContainer
{
	public BlockLDoor(int par1)
	{
	...
	public void setBlockBoundsForItemRender() {this.setBlockBounds(0, 0, 0, 1, 1f, .2f);}
	/**
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) 
    {
    	...
    	if(meta < 8)
    	{
	    	...
	    	if( meta < 4) 
	    	{
	    		switch(meta)
	    		{
		    		case 2:setBlockBounds(0, 0, 0, 1, 2, .2f);break;
		    		case 3:setBlockBounds(.8f, 0, 0, 1, 2, 1);break;
		    		case 0:setBlockBounds(0, 0, .8f, 1, 2, 1);break;
		    		case 1:setBlockBounds(0, 0, 0, .2f, 2, 1);break;
	    		}
	    	}
	    	else if(meta > 3 && meta < 8)
	    	{
	    		if(aboveMeta == 14)
	    		{
	    			switch(meta)
	    			{
	    				case 5:setBlockBounds(0, 0, 0, 1, 2, .2f);break;
	    				case 6:setBlockBounds(.8f, 0, 0, 1, 2, 1);break;
	    				case 7:setBlockBounds(0, 0, .8f, 1, 2, 1);break;
	    				case 4:setBlockBounds(0, 0, 0, .2f, 2, 1);break;
	    			}
	    		}else
	    		{
	    			switch(meta)
	    			{
	    				case 7:setBlockBounds(0, 0, 0, 1, 2, .2f);break;
	    				case 4:setBlockBounds(.8f, 0, 0, 1, 2, 1);break;
	    				case 5:setBlockBounds(0, 0, .8f, 1, 2, 1);break;
	    				case 6:setBlockBounds(0, 0, 0, .2f, 2, 1);break;
	    			}
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
    {
       ...
       if(meta > 13 && te instanceof TileEntityLDoor)
       {
    	   boolean canOpen = ((TileEntityLDoor)te).canOpen(player);
         ...
         {
        	  if(world.getBlockTileEntity(x, y+1, z) instanceof TileEntityLDoor)
        	  {
	...
	public void onBlockPlacedBy(World par1World, int i, int j, int k, EntityLiving par7E)
    {
		...
		if(te instanceof TileEntityLDoor && par7E instanceof EntityPlayer)
		{
			((TileEntityLDoor)te).setOwner((EntityPlayer) par7E);
		}
    }
	...
	public TileEntity createNewTileEntity(World par1World,int meta)
    {
		if(meta >= 0 && meta < 8)
		{
			return new TileEntityLDoorTop();
		}else
		if(meta > 13)
		{
			return new TileEntityLDoor();
		}
		return null;
    }
