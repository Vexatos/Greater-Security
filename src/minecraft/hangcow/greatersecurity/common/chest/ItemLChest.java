...
public class ItemLChest extends ItemBlock
{

    private int blockID = GreaterSecurity.BlockLChest.blockID;

    public ItemLChest(int par1)
    {
        super(par1);
        this.setIconIndex(0);
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int i, int j, int k, int side, float hitX, float hitY, float hitZ, int metadata)
    {
        for (int s = 0; s < 4; s++)
        {
            ...
            if (e instanceof TileEntityLChest)
            {
                int bType = ((TileEntityLChest) e).getType();
                if (world.getBlockId(i + deltaX, j, k + deltaZ) == this.blockID && bType != stack.getItemDamage()) { return false; }
            }
        }
        ...
        TileEntityLChest blockEntity = (TileEntityLChest) world.getBlockTileEntity(i, j, k);
        ...
        if (!world.isRemote && player != null && blockEntity != null)
        {
            ...
            for (int s = 0; s < 4; s++)
            {
                ...
                if (C instanceof TileEntityLChest)
                {
                    if (((TileEntityLChest) C).getType() == stack.getItemDamage())
                    {
                        TileEntityLChest LC = (TileEntityLChest) world.getBlockTileEntity(i + x, j, k + z);
                        ((TileEntityLChest) blockEntity).setOwnerString(LC.BlockOwner);
                        ((TileEntityLChest) blockEntity).users = LC.users;
                ...
                if (spawnNormal)
                {
                    ...
                    ((TileEntityLChest) blockEntity).setOwner((EntityPlayer) player);
                }
            ...
            if (debug)
            {
                ItemStack[] chestInv = ((TileEntityLChest) blockEntity).chestContents;
