...
public class TileEntityLChest extends TileEntity implements IInventory, IPacketReceiver
{
    ...
    public TileEntityLChest[] chests =
    { null, null, null, null };
    ...
    @Override
    public void handlePacketData(INetworkManager network,
    ...
    public void checkForAdjacentChests()
    {
        if (!this.adjacentChestChecked)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (entity instanceof TileEntityLChest)
                {
                    if (((TileEntityLChest) entity).getType() == this.getType())
                    {
                        this.chests[i] = (TileEntityLChest) entity;
                    }
    ...
    public TileEntityLChest getAdjacentChest()
    {
        for (int i = 0; i < 4; i++)
        {
            int deltaX = 0;
            ...
            if (entity instanceof TileEntityLChest)
            {
                if (((TileEntityLChest) entity).getType() == this.getType()) { return (TileEntityLChest) entity; }
            }
        }
        return null;
    ...
    public boolean canAccess(TileEntityLChest chest, EntityPlayer player)
    {
        if (this.BlockOwner.equalsIgnoreCase("World")) { return true; }
        if (chest instanceof TileEntityLChest)
        {
            ...
            if (player == owner || pEntity.contains(player)) { return true; }
        }
        return false;
    }
    ...
    {
        TileEntityLChest aChest = this.getAdjacentChest();

        if (aChest instanceof TileEntityLChest)
        {
    ...
    public void removeUser(String user)
    {
        ...
        TileEntityLChest lc = this.getAdjacentChest();
        if (lc instanceof TileEntityLChest)
        {
    ...
    public void updateEntity()
    {
        ...
        if (++this.ticksSinceSync % 20 * 4 == 0)
        {
            ...
            {
                PacketManager.sendChestPacketPacket(this, 0, this.BlockOwner, users);
            }
